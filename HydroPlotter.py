...
class HydroPlotter(object):
    ...
    def TimeSeriesPlot(self,Dates,Values,Var_LUn,Title=None,Name='Test.png',PathImg='',Labels=None,
            Colors=None,Linestyles=None,Date_Format='%Y/%m',**args):
        '''
        DESCRIPTION:
        
            This method generates daily temporal plots. If Values is a matrix
            the function would plot each line. However the Label is required
            for this usage

        _________________________________________________________________________

        INPUT:
            :param Dates: A ndarray, Vector with dates in datetime format.
            :param Values: A ndarray, Vector or matrix with values to plot.
            :param Var_LUn: Label label of the y axis.
            :param Title: Title, if None the title would not get displayed.
            :param PathImg: A str, Directory where the image would be saved, if the
                            directory doesn't exists the method would create it.'
            :param Name: A str, Name of the image with extension, Ej. 'test.png.'
            :param Labels: A list, list with all the labels of the Values data.
            :param Colors: A list, list with all the Colors of the Values data.
            :param LineStyles: A list, list with all the line styles of the Values 
            :param Date_Format: A str, Date format that would show in the x axis.
            :param **args: Additional arguments for the plot.
        _________________________________________________________________________
        ...
        # Graph generation
        F = plt.figure(figsize=DM.cm2inch(self.fH,self.fV))
        # Figure Parameters
        ...
        if len(Values.shape) == 2:
            for i in range(Values.shape[0]):
                if not(Colors is None) and Linestyles is None:
                    plt.plot(Dates,Values[i],color=Colors[i],label=Labels[i])
                elif Colors is None and not(Linestyles is None):
                    plt.plot(Dates,Values[i],linestyle=Linestyles[i],label=Labels[i])
                elif not(Colors is None) and not(Linestyles is None):
                    plt.plot(Dates,Values[i],color=Colors[i],linestyle=Linestyles[i],label=Labels[i])
                else:
                    plt.plot(Dates,Values[i],label=Labels[i])
        else:
        ...
        # Changing Dates label
        xTL = axes.xaxis.get_ticklocs() 
        Labels2 = DUtil.Dates_datetime2str(Dates,Date_Format)
        plt.xticks(xTL, Labels2) 
        # rotating axis
        ...
        plt.tight_layout()
        # Figure Saving
        plt.savefig('{}{}'.format(PathImg,Name),format=Name.split('.')[-1],dpi=self.dpi)
        plt.close('all')
    ...
    def CompS(self,Date,V1,V2,Lab1,Lab2,Var_LUn,Var='',flagT=True,v='',PathImg=''):
        ...
        axs[1].tick_params(axis='x',which='both',bottom=True,top=False,\
            labelbottom=True,direction='out')
        axs[1].tick_params(axis='y',which='both',left=True,right=False,\
            labelleft=True)
